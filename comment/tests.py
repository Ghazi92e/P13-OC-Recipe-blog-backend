...
class CommentTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='User1', email='user1@mail.com', password='user123', file=file, image_url='user_image_url')
        self.client.force_authenticate(user=self.user)

        self.comment = Comment.objects.create(description="MyFirstComment", user=self.user)
    ...
    def test_can_update_comment(self):
        serializer = CommentSerializer(self.comment)
        pk = self.comment.pk
        response = self.client.put(self.comment_path + str(pk) + '/', serializer.data)
