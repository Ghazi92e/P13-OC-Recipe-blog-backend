...
class UsersTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.recipe = Recipes.objects.create(title='Recette1', ingredients='Ingredients1', description='Description1', file=file, user=self.user, category=self.cat1, image_url='http://127.0.0.1:8000/media/recipes1')

        self.user2 = User.objects.create(username='test2', email='test2@mail.com', password='user123', file=file, image_url='http://127.0.0.1:8000/media/user2')
    ...
    def test_can_read_user_following_recipes(self):
        pk = self.user.pk
        response = self.client.get(self.users_path + str(pk) + '/recipes_user_following/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_read_user_following_recipes_by_category(self):
        ...
        pk = self.user.pk
        response = self.client.post(self.users_path + str(pk) + '/get_user_recipes_following_by_category/', product_category)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_create_user_following(self):
        ...
        pk = self.user.pk
        response = self.client.post(self.users_path + str(pk) + '/following/', create_user_following)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_can_delete_user_following(self):
        ...
        response = self.client.delete(self.users_path + str(pk) + '/following/', user_following)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_can_search_recipes_user_following(self):
        ...
        response = self.client.post(self.users_path + 'search_recipes_by_user_followings/', search_recipes)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_count_user_follower(self):
        pk = self.user.pk
        response = self.client.get(self.users_path + str(pk) + '/count_user_follower/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_can_count_user_following(self):
        pk = self.user.pk
        response = self.client.get(self.users_path + str(pk) + '/count_user_following/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    
    def test_can_count_user_recipes(self):
        pk = self.user.pk
        response = self.client.get(self.users_path + str(pk) + '/count_user_recipes/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
